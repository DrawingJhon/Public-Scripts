script = script or Instance.new("Script")
...
function require(module)
	...
	return RequireCache[name] or (function()
		local result = func()
...
for _,v in next, Dependencies do script:WaitForChild(v) end
...
local function exec_lua_func(exst)
	...
	while true do
		...
		if _G.stop then error("Script ended") end
		if op < 19 then
			if op < 9 then
				if op < 4 then
					if op < 2 then
...
do
	...
	for i, v in pairs(Dependencies) do
		Instance.new("ModuleScript", script).Name = v
		local SavedVals = {}
	end
end

return assert(require(script)(_G.source, getfenv()))()