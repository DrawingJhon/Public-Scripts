--FE Compatiblity by DrawingJhon
--I hope I have fixed all the bugs and errors in the script
...
if game:GetService("RunService"):IsClient() then
	error("Cannot run this script in client sided, say 'h/' and not 'hl/'")
end

script.Name = "MainReplicator"

_G.target = owner.Name
local asdfekiruasc = Instance.new("Folder")
asdfekiruasc.Name = "MInfo"
local l1ll1l1ll1l1l1 = {}
for i = 1, 1000 do
	table.insert(l1ll1l1ll1l1l1, "r"..tostring(i))
end
for i, v in next, l1ll1l1ll1l1l1 do
	--Instance.new("BindableEvent", asdfekiruasc).Name = v
end

local nsl1l1l1l1l1l = NS([===[
local Plr = game.Players:WaitForChild(_G.target) --Change script.Parent.Parent to "UsernameHere" but replace usernamehere with your actual roblox username
local User = Plr.Name
...
MainFdr.Name = Plr.Name
MainFdr.Parent = RepStorage
...
F.Visible = false
F.Parent = FakeChar
FakeChar.Parent = MainFdr

local Mouse = Instance.new("BindableEvent")
Mouse.Name = "Mouse"
Mouse.Parent = MainFdr
local MouseInformation = script:WaitForChild("MInfo")
...
--[[local CurrentModeRem = Instance.new("BindableEvent")
...
local Movement = Instance.new("BindableEvent")
Movement.Name = "MovingEvent"
Movement.Parent = MainFdr

local DamageRemote = Instance.new("BindableEvent")
...
local FinishRemote = Instance.new("BindableEvent")
FinishRemote.Name = "EndRemote"
FinishRemote.Parent = MainFdr

for i,v in pairs(workspace.Terrain:GetChildren()) do
	if v.Name == Plr.Name.." char tracker" then
		v:Destroy()
	end
end

local FakeRoot = Instance.new("Part")
FakeRoot.Name = Plr.Name.." char tracker"
FakeRoot.CanCollide = false
FakeRoot.Size = FakeChar.HumanoidRootPart.Size
FakeRoot.Transparency = 1
FakeRoot.Parent = workspace.Terrain
...
FakeRoot:SetNetworkOwner(Plr)

for i,v in pairs(MouseInformation:GetChildren()) do
	v.Event:Connect(function(Position, TARG)
		mPos.Value = Position
		mTarget.Value = TARG
	end)
end
...
Plr.Chatted:Connect(function(msg)
	if string.sub(msg,1,7) == "=>Loop " then
		ChangeLoop(string.sub(msg, 8))
	end
	if string.sub(msg,1,7) == "=>Move " then
		ChangeMovement(string.sub(msg, 8))
	end
	if msg == "==>End" then
		FinishRemote:Fire()
		wait()
...
function ApplyDamage(Humanoid, Damage, DAMAGEMULTIPLIER)
	print(Humanoid, Damage, DAMAGEMULTIPLIER)
...
DamageRemote.Event:Connect(function( request, ...)
...
--[[CurrentModeRem.Event:Connect(function(request, ...)
	CurrentModeVal.Value = ...
end)--]]
...
script.MainReplicator.Name = Plr.Name
--[[for i,v in pairs(game:GetService("Players"):GetChildren()) do
	local NS = script[Plr.Name]:Clone()
...
--script.LoadLibrary.Parent = RepStorage
-- Synced Loop 
...
AHB.Event:Connect(function()
	pcall(function()
		--Plr.Character.HumanoidRootPart.CFrame = CFrame.new(10000,10000,0)
	end)
end)
...
while true do
	AHB.Event:wait()
	game:GetService("Debris"):AddItem(Instance.new("Shirt"),0)
end
]===], owner.PlayerGui)
wait(0.1)
asdfekiruasc.Parent = nsl1l1l1l1l1l
script.Parent = nsl1l1l1l1l1l

local nls = NLS([==[
local plr = game:GetService("Players").LocalPlayer
for i, v in pairs(plr.PlayerGui:GetChildren()) do
	if v:IsA("LocalScript") and v.Name == "LSJoy" then
		v:Destroy()
	end
end
script.Name = "LSJoy"
local mouse = plr:GetMouse()
local KeyDown = plr:WaitForChild("KeyDown")
local KeyUp = plr:WaitForChild("KeyUp")
local B1Down = plr:WaitForChild("B1Down")
local B1Up = plr:WaitForChild("B1Up")
local MouseInfo = plr:WaitForChild("MouseInfo")
local char
...
conn = game:GetService("RunService").RenderStepped:Connect(function()
	MouseInfo:FireServer({
		Hit = mouse.Hit,
		Target = mouse.Target
	})
	if char and char.Parent ~= nil and char:findFirstChildOfClass("Humanoid") then
		workspace.CurrentCamera.CameraSubject = char:findFirstChildOfClass("Humanoid")
		--owner.Character = char
	end
end)
local charVal = script:WaitForChild("ThisChar")
char = charVal.Value
]==], owner.PlayerGui)
...
char = MainFolder:findFirstChildOfClass("Model"):Clone() -- Character clone with model
FakeRoot = workspace.Terrain:WaitForChild(Player.Name.." char tracker")
--local WeldRemote = MainFolder.WeldRemote

StopRemote.Event:Connect(function()
	if lplr == Player then
...
if lplr == Player then
	...
	local KeyDown = Player:findFirstChild("KeyDown") or Instance.new("RemoteEvent", Player)
	KeyDown.Name = "KeyDown"
	local KeyUp = Player:findFirstChild("KeyUp") or Instance.new("RemoteEvent", Player)
	KeyUp.Name = "KeyUp"
	local B1Down = Player:findFirstChild("B1Down") or Instance.new("RemoteEvent", Player)
	B1Down.Name = "B1Down"
	local B1Up = Player:findFirstChild("B1Up") or Instance.new("RemoteEvent", Player)
	B1Up.Name = "B1Up"
	local MouseInfo = Player:findFirstChild("MouseInfo") or Instance.new("RemoteEvent", Player)
	MouseInfo.Name = "MouseInfo"
	RealMouse = {}

	MouseInfo.OnServerEvent:Connect(function(plr, self)
		RealMouse.Hit = self.Hit
		RealMouse.Target = self.Target
	end)
	KeyDown.OnServerEvent:Connect(function(plr, Key)
		Remote:Fire("KeyDown", Key)
	end)
	KeyUp.OnServerEvent:Connect(function(plr, Key)
		Remote:Fire("KeyUp", Key)
	end)
	B1Down.OnServerEvent:Connect(function(plr, Key)
		Remote:Fire("MouseButton1Down", Key)
	end)
	B1Up.OnServerEvent:Connect(function(plr, Key)
		Remote:Fire("MouseButton1Up", Key)
	end)
	
	spawn(function()
		while true do
			MouseInfo.OnServerEvent:Wait()
			for i = 1,1000,1 do
				pcall(function()
					HB:wait()
					Remote2["r"..i]:Fire(RealMouse.Hit, RealMouse.Target)
				end)
			end
		end
	end)
...
do
	...
	Remote.Event:Connect(function(request, ...)
	...
	--[[for i,v in pairs(Hits:GetChildren()) do
		v.Event:Connect(function(HIT, TARGET)
			Hit = Hit
			Target = Target
		end)
	end--]]
...
local lastPosRoot = Vector3.new()
if lplr == Player then
	game:GetService("RunService").Heartbeat:Connect(function()
		Player.Character = char
		local root = char:findFirstChild("HumanoidRootPart")
		if root then
			lastPosRoot = root.Position
		end
		Cam.CameraSubject = char:FindFirstChildOfClass("Humanoid")
	end)
end
...
function SaveSound(name, parent)
	local sound = getfenv()[name]
	local looped = sound.Looped
	local playing = false
	local timePos = 0
	local start
	local function onChanged(s)
		local success, err = pcall(function()
			s.Parent = parent
		end)
		if not success then start() return true end
	end
	local function pl(s)
		local conn
		conn = game:GetService("RunService").Heartbeat:Connect(function()
			if s.Parent ~= parent then conn:Disconnect() return end
			timePos = s.TimePosition
		end)
		if s.IsPlaying or s.Playing then
			playing = true
		else
			playing = false
		end
		...
		mt.__index = function(self, k)
			if type(s[k]) == "function" then
				if k:lower() == "stop" then
					playing = false
				elseif k:lower() == "play" then
					playing = true
				elseif k:lower() == "pause" then
					playing = false
				end
	...
	function start()
		...
		if not success then
			warn("Error trying to retrieve sound '"..name.."'")
			repeat wait()
				pcall(function()
					clone = sound:Clone()
				end)
			until clone
			print("The sound '"..name.."' was replayed correctly")
		end
		clone.Looped = looped
		clone.Parent = parent
		clone.TimePosition = timePos
		if playing then
			clone:Play()
		end
		pl(clone)
		local conn
		conn = clone.Changed:Connect(function()
			if onChanged(clone) then conn:Disconnect() end
		end)
	end
	pl(sound)
	local conn
	conn = sound.Changed:Connect(function()
		if onChanged(sound) then conn:Disconnect() end
	end)
end
...
BTAUNT2.Pitch = 1
BTAUNT2.Looped = true
SaveSound("BTAUNT2", hed)

BTAUNT3 = Instance.new("Sound", char) -- What is this sound?
...
--End Attacks N Stuff--
-------------------------------------------------------
Player.KeyDown.OnServerEvent:connect(function(plr, key)
...
if lplr ~= Player then
	...
	Mover.Event:Connect(function(v, Speed)
...
--Start Animations--
-------------------------------------------------------
print("By XandersAltAccount1 / FE Compatiblity by DrawingJhon")
while true do
	swait()
	sine = sine + change
