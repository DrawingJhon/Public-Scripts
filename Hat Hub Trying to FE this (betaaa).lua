--//Hathub compatibility to vsb and FE by DrawingJhon
-- Not needs to buy hats YEAH

assert(owner and typeof(owner) == "Instance" and owner:IsA("Player"), "Not owner found")
local Instance = {new = Instance.new}

for _, child in pairs(owner.Character and owner.Character:GetChildren() or {}) do
	if child:IsA("Accessory") and not child.Name:match("__") then
		child.Name = "__"..child.Name
	end
end
--[[local hats = {"Nagamaki", "Robloxclassicred", "Pal Hair", "Pink Hair", "Hat1", "Kate Hair", "LavanderHair", "Bedhead", "BlockheadBaseballCap", "MessyHair"}
for i, name in pairs(hats) do
	local ac = Instance.new("Accessory")
	ac.Name = name
	local handle = Instance.new("Part", ac)
	handle.Name = "Handle"
	handle.BottomSurface = "Smooth"
	handle.TopSurface = "Smooth"
	handle.Size = (name == "Nagamaki" and Vector3.new(5, 3, 1) or Vector3.new(1, 1, 2))
	handle.CanCollide = false
	local mesh = Instance.new("SpecialMesh", handle)
	mesh.Name = "Mesh"
	mesh.MeshType = "FileMesh"
	local at = Instance.new("Attachment", handle)
	at.Name = "HatAttachment"
	local vs = Instance.new("Vector3Value", handle)
	vs.Value = (name == "Nagamaki" and Vector3.new(5, 3, 1) or Vector3.new(1, 1, 2))
	vs.Name = "OriginalSize"
	local weld = Instance.new("Weld", handle)
	weld.Name = "AccessoryWeld"
	weld.Part0 = handle
	weld.Part1 = owner.Character.Head
	ac.Parent = owner.Character
end]]
...
do
	...
	print("Rescripted by DrawingJhon and made this compatibility to vsb and serverside.z")
	...
	local FakeGame = {
		...
		Players = FakeService({
			LocalPlayer = FakeService({GetMouse=function(self)return FakeMouse end, MaximumSimulationRadius = 100},Player)
		},"Players"),
...
do
	...
	local fakeSettings = {}
	local oof
	function oof(tab, name)
		return setmetatable(tab, {
			__index = function(self, index)
				rawset(tab, index, oof({}, index))
				return rawget(tab,index)
			end;
			__tostring = tostring(name);
		})
	end
	oof(fakeSettings, "settings")
	function settings()
		return fakeSettings
	end
	function getgenv()
...
function SetHats(name) -- This function has been added by me, then it's not necessary to buy the hats, I did it free ;)
	...
	if HatString[name] and HatString[name] ~= "" then
		...
		for i, info in next, data do
			...
			handle.BottomSurface = "Smooth"
			handle.TopSurface = "Smooth"
			handle.Size = toVector3(info.Size)
			handle.CanCollide = false
			handle.Transparency = 0
			local mesh = Instance.new("SpecialMesh", handle)
			mesh.Name = info.Mesh.Name
			mesh.MeshType = "FileMesh"
			mesh.MeshId = info.Mesh.MeshId
			mesh.TextureId = info.Mesh.TextureId
			mesh.Offset = toVector3(info.Mesh.Offset)
			mesh.Scale = toVector3(info.Mesh.Scale)
			local at = Instance.new("Attachment", handle)
			at.Name = "HatAttachment"
			if info.OriginalSize then
				local vs = Instance.new("Vector3Value", handle)
				vs.Name = "OriginalSize"
				vs.Value = toVector3(info.OriginalSize)
			end
			if info.AvatarPartScaleType then
				local val = Instance.new("StringValue", handle)
				val.Name = "AvatarPartScaleType"
				val.Value = info.AvatarPartScaleType
			end
			if info.Fire then
				local fire = Instance.new("Fire", handle)
				fire.Name = info.Fire.Name
				fire.Color = toColor3(info.Fire.Color)
				fire.Enabled = info.Fire.Enabled
				fire.SecondaryColor = toColor3(info.Fire.Color)
				fire.Heat = info.Fire.Heat
				fire.Size = info.Fire.Size
			end
			if info.Sparkles then
				local sparkle = Instance.new("Sparkles", handle)
				sparkle.Name = info.Sparkles.Name
				sparkle.Enabled = info.Sparkles.Enabled
				sparkle.SparkleColor = toColor3(info.Sparkles.SparkleColor)
			end
			ac.Parent = char
		end
	else
		warn("Does not exist: "..tostring(name))
	end
end
...
StrongStand.MouseButton1Down:connect(function()
	local toggle
    SetHats("Strong Stand")
    local m = Instance.new("Message")

m.Parent = game:GetService("Players").LocalPlayer:findFirstChildOfClass("PlayerGui")
...
local handle = game.Players.LocalPlayer.Character:FindFirstChild("Torso")
